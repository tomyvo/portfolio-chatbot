...
def get_session_history(session_id: str):
    # Use sync connection for simplicity in serverless, or manage async properly.
    # PostgresChatMessageHistory from langchain_postgres handles sync/async
    return PostgresChatMessageHistory(
        connection_string=POSTGRES_URL,
        session_id=session_id,
        table_name="chat_history" # Ensure this table exists or is created
    )
...
async def chat_endpoint(request: ChatRequest):
    ...
    if not POSTGRES_URL:
        raise HTTPException(status_code=500, detail="Postgres URL not configured")
